<?php include_once('./inc/inc-html.php'); // เรียกใช้ไฟล์ HTML ที่จำเป็น

if ($_SERVER['REQUEST_METHOD'] === "POST") { // ตรวจสอบว่าคำขอเป็นแบบ POST หรือไม่

    $rp = false; // กำหนดตัวแปรตรวจสอบผลลัพธ์

    if (isset($_POST["TYPE"]) && $_POST["TYPE"] === "EDIT") { // ตรวจสอบว่าคำขอเป็นการแก้ไขข้อมูล

        $USER_ID = $gUSER_ID; // กำหนดค่า ID ของผู้ใช้

        // ดึงข้อมูลผู้ใช้จากฐานข้อมูลเพื่อตรวจสอบว่ามีอยู่จริงหรือไม่
        ...
        $query = $conn->query($sql); // ทำการอัพเดตข้อมูล

        if ($query) { // ตรวจสอบว่าการอัพเดตสำเร็จหรือไม่
            $rp = true;
        }
    }

    if ($rp) { // ถ้าอัพเดตสำเร็จ แสดงข้อความแจ้งเตือนและเปลี่ยนเส้นทาง
        echo "
            <script>
        ...
        exit;
    } else { // ถ้าอัพเดตไม่สำเร็จ แสดงข้อความแจ้งเตือนและกลับไปหน้าเดิม
        echo "
        <script>
        ...
        <div class="container">
            <div id="profile" class="profile-container">
                ...
                <div class="profile-info mb-4">
                    ...
                    <p><strong>ที่อยู่:</strong> <?= $r_user['USER_ADDRESS']; ?> </p>
            ...
            <div class="edit-form" id="editForm" style="display: none;">
                <form id="form-login" action="./profile.php" method="post" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-12">
                            <div class="form-group ">
                                ...
                                <textarea required class="form-control mt-0" name="USER_ADDRESS"
                                    rows="3"><?= $r_user['USER_ADDRESS']; ?></textarea>
                            </div>
